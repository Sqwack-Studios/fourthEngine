...
_Use_decl_annotations_
inline XMFLOAT3 BoundingBox::Diagonal() const noexcept
{
    XMVECTOR center = XMLoadFloat3(&Center);
    XMVECTOR extents = XMLoadFloat3(&Extents);

    XMVECTOR vDiagonal = XMVectorAdd(center, extents);

    XMFLOAT3 returnDiagonal;

    XMStoreFloat3(&returnDiagonal, 2.0f * vDiagonal);
...
_Use_decl_annotations_
inline BoundingBox BoundingBox::MinMaxToExtentsCenter(_In_ XMFLOAT3 min, _In_ XMFLOAT3 max) noexcept
{
    ...
    XMStoreFloat3(&extents, vExtents);
...
namespace TriangleTests
{
    ...
    _Use_decl_annotations_
        inline bool XM_CALLCONV Intersects(
            FXMVECTOR Origin, FXMVECTOR Direction, FXMVECTOR V0,
            GXMVECTOR V1,
            HXMVECTOR V2, float& Dist, PlaneIntersectionType& outFaceIntersected) noexcept
    {
        assert(DirectX::Internal::XMVector3IsUnit(Direction));
        ...
        if (XMVector3GreaterOrEqual(det, g_RayEpsilon))
        {
            ...
            outFaceIntersected = PlaneIntersectionType::FRONT;
        }
        else if (XMVector3LessOrEqual(det, g_RayNegEpsilon))
        {
            // Determinate is negative (back side of the triangle).
            ...
            outFaceIntersected = PlaneIntersectionType::BACK;
        ...
        else
        {
            // Parallel ray.
            Dist = 0.f;
            outFaceIntersected = PlaneIntersectionType::NUM;
            return false;
        }
    ...
    _Use_decl_annotations_
        inline bool XM_CALLCONV Intersects(
            FXMVECTOR Origin, FXMVECTOR Direction, FXMVECTOR V0,
            GXMVECTOR V1,
            HXMVECTOR V2, XMFLOAT3& outNormal, float& Dist) noexcept
    {
        assert(DirectX::Internal::XMVector3IsUnit(Direction));
        ...
        else
        {
            ...
            Dist = 0.f;
            return false;
            XMStoreFloat3(&outNormal, XMVectorZero());
        }
    ...
    _Use_decl_annotations_
        inline bool XM_CALLCONV Intersects(
            FXMVECTOR Origin, FXMVECTOR Direction, FXMVECTOR V0,
            GXMVECTOR V1,
            HXMVECTOR V2, XMFLOAT3& outNormal, float& Dist, PlaneIntersectionType& outFaceIntersected) noexcept
    {
        assert(DirectX::Internal::XMVector3IsUnit(Direction));
        ...
        XMStoreFloat3(&outNormal, XMVector3Cross(e1, e2));
