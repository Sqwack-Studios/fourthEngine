...
_Use_decl_annotations_
HRESULT DirectX::LoadDDSFromFileEx(
    ...
    size_t& bitSize) noexcept
{
    ...
    std::unique_ptr<uint8_t[]> headerData;
    ...
    // create enough space to read the header
    headerData.reset(new (std::nothrow) uint8_t[(sizeof(uint32_t) + sizeof(DDS_HEADER))]);
    if (!headerData)
    {
        return E_OUTOFMEMORY;
    }
    ...
    // read the data in
    {
        ...
        if (bytesRead < sizeof(uint32_t) + sizeof(DDS_HEADER))
        {
            headerData.reset();
            return E_FAIL;
        }
        ...
        // DDS files always start with the same magic number ("DDS ")
        auto const dwMagicNumber = *reinterpret_cast<const uint32_t*>(headerData.get());
        if (dwMagicNumber != DDS_MAGIC)
        {
            headerData.reset();
            return E_FAIL;
    ...
    auto hdr = reinterpret_cast<const DDS_HEADER*>(headerData.get() + sizeof(uint32_t));
    ...
    if (bDXT10Header)
    {
        //Reset current header data, and read again.
        headerData.reset(new (std::nothrow) uint8_t[offset]);
        if (!headerData)
        {
            return E_OUTOFMEMORY;
        }
    ...
    //Verify if its cubemap
    if (bDXT10Header)
    {
        auto d3d10ext = reinterpret_cast<const DDS_HEADER_DXT10*>(reinterpret_cast<const uint8_t*>(hdr) + sizeof(DDS_HEADER));
    ...
    {
        format = GetDXGIFormat(hdr->ddspf);
    ...
    if (isCubemap)
    {
        if ((width > D3D11_REQ_TEXTURECUBE_DIMENSION) || (height > D3D11_REQ_TEXTURECUBE_DIMENSION))
        {
            return HRESULT_FROM_WIN32(ERROR_NOT_SUPPORTED);
        }
    }
    else if ((width > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION) || (height > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION))
    {
        return HRESULT_FROM_WIN32(ERROR_NOT_SUPPORTED);
    }
    ...
    //Allocate DDS data
    ddsData.reset(new (std::nothrow) uint8_t[fileInfo.EndOfFile.LowPart - offset]);
    if (!headerData)
    {
        return E_OUTOFMEMORY;
    }

    {
        ...
        if (!ReadFile(hFile.get(),
            ddsData.get(),
            fileInfo.EndOfFile.LowPart - offset,
            &bytesRead,
            nullptr
        ))
        {
        ...
        if (bytesRead < fileInfo.EndOfFile.LowPart - offset)
        {
            ddsData.reset();
            return E_FAIL;
        }
...
HRESULT DirectX::FillInitData(
    ...
    size_t& tdepth,
    size_t& skipMip,
    D3D11_SUBRESOURCE_DATA* initData) noexcept
{
    if (!bitData || !initData)
    {
        return E_POINTER;
