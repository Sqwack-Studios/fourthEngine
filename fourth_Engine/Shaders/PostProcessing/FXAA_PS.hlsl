...
Texture2D<float4> g_image 		: register(srv_LDR_FXAA); // .rgb = LDR sRGB color (after gamma-correction), .a = color luma
//SamplerState g_bilinearClamp 	: register(g_bilinearClamp);

cbuffer cb_local : register(cb_FXAA_RESOLVE)
{
...
float4 main(PSIn pin) : SV_TARGET
{
	...
	return FxaaPixelShader(
		pin.pos.xy * g_imageSize.zw, // map pixel coordinate to [0; 1] range
